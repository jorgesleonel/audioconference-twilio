...
// Obtain information related to the audio conference
if($result = mysqli_query($link, "SELECT * FROM users WHERE user_ID = '$UID' LIMIT 1") ){
		if($data = mysqli_fetch_assoc($result)){
			...
			// Obtain participant count for each audio conference, and then insert into an array object
			...
			$call_history = "";
			// Cycle over the list of conferences and print a property of each one
			foreach ($client->account->conferences->getIterator(0, 50, array(
				...
				$date = DateTime::createFromFormat('D, d M Y H:i:s P', $conference->date_updated);
				$call_history .= "<tr><td>" . $date->format("Y-m-d H:i:s") . "</td><td>$conference->status</td><td>$durationText</td><td>$count</td><td>$name_recordings</td><td>$call_recordings</td></tr>";
			}
			...
			foreach ($client->account->conferences->getIterator(0, 50, array(
				...
				foreach($conference->participants as $participant){
					...
					if( $call->to == $call->from ) continue; // Ignore the announcers
